        ...
        [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "upload")] HttpRequest req)
    {
        ...
        if (!isStripped)
        {
            try
            {
                ...
                var response = await _httpClient.PostAsJsonAsync("http://localhost:8000/api/math/strip-ka2", pyRequest);
                
                if (response.IsSuccessStatusCode)
                {
                    var pyResult = await response.Content.ReadFromJsonAsync<JsonElement>();
                    
                    var cleanedArray = pyResult.GetProperty("cleaned_intensity").EnumerateArray()
